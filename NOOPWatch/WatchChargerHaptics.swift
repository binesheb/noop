...
///
/// Battery monitoring is enabled before the first state read. The initial state is seeded silently so an
/// already-charging Watch does not look like a new connection. Subsequent changes are reduced through
/// `WatchChargerHapticPolicy` before touching the haptic engine.
